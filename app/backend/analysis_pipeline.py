...
class AnalysisPipeline:
    ...
    def load_files(self, filepath: str):
        ...
        if "status" not in fm_result:
            raise RuntimeError("FileManager returned an unexpected response.")
    ...
    def save_results(self, data_bundle, results_bundle, analysis_id: str, return_id: bool = False) -> str:
        try:
            ...
            raise RuntimeError(f"Failed to save results to database: {e}")
    ...
    def run_metadata_analysis_pipeline(self, text_nodes, code_nodes, binary_data):
        ...
        if metadata_analysis and metadata_analysis != {}:
            ...
            self.cli.print_status("Metadata analysis returned no results.", "error")
            return {}, {}
    ...
    def run_repo_analysis_pipeline(self, git_repos, binary_data):
        ...
        if github_username:
            user_email: str = self.cli.get_input("  Enter the GitHub email for this account:\n> ").strip().lower()
            ...
            try:
                ...
                if not processed_git_repos:
                    ...
                    if not analyzed_repos:
                        ...
                        self.cli.print_status(f"{len(analyzed_repos)} repo(s) analysed.", "success")
                        self.cli.print_status("Ready to rank projects.", "info")
    ...
    def run_AI_NLG(self, data_bundle, result_bundle, text_analysis_data):
        try:
            ...
            if online_consent:
                self.cli.print_status("Using online LLM.", "success")
                try:
                    llm_client = OnlineLLMClient()
                except ValueError as e:
                    self.cli.print_status(f"Online LLM initialisation failed: {e}  Falling back to local.", "error")
                    llm_client = LocalLLMClient()
            else:
                self.cli.print_status("Using local LLM.", "success")
                llm_client = LocalLLMClient()

            self.cli.print_status("Generating summary — this may take a moment...", "info")

            try:
                result_bundle.medium_summary = llm_client.generate_summary(topic_vector_bundle)
                self.cli.print_header("Generated Summary")
            ...
            raise RuntimeError(f"Failed to collect statistics: {e}")
    ...
    def run_analysis(self, filepath: str, return_id=False, existing_analysis_id: Optional[str] = None, preloaded_tree: Optional[Node] = None, preloaded_binary: Optional[List[bytes]] = None) -> None | str:
        """
        Runs the analysis pipelines in sequence.
        Early steps abort on error; individual pipeline errors are reported and execution continues.
        """
        filetree: Node = None
        ...
        if preloaded_tree and preloaded_binary:
            ...
            try:
                ...
                self.cli.print_status(f"Failed to initialise analysis in database: {e}", "error")
                return

        try:
            textfile_nodes, codefile_nodes, git_repos, binary_data = self.classify_files(filetree, binary_data)
        except Exception as e:
            self.cli.print_status(f"File classification failed — aborting: {e}", "error")
            return
        ...
        try:
            ...
            self.cli.print_status(f"Content analysis failed: {e}", "error")
